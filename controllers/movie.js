...
exports.createMovie = async (req, res) => {
  ...
  res.status(201).json({
    movie: { id: newMovie._id, title, message: "Movie uploaded Successfully!" },
  });
};
...
exports.updateMovieWithoutPoster = async (req, res) => {
  ...
  res.json({ message: "Movie has been updated!", movie });
};
...
exports.removeMovie = async (req, res) => {
  ...
  // if yes destroy it from cloudinary
  const posterId = movie.poster?.public_id;
  if (posterId) {
    const { result } = await cloudinary.uploader.destroy(posterId);
    // console.log(result);
...
exports.getLatestUploads = async (req, res) => {
  ...
  const results = await Movie.find().sort("-createdAt").limit(limit);
...
exports.getSingleMovie = async (req, res) => {
  ...
  if (!movie)
    return sendError(
      res,
      "Sorry! we are unable to find this movie at the moment!"
    );
