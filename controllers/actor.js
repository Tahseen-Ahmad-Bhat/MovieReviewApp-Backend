...
exports.updateActor = async (req, res) => {
  ...
  if (!isValidObjectId(actorId)) return sendError(res, "Invalid request!", 404);
  ...
  // upload new avatar if there is new image
  if (file) {
    const { public_id, url } = await uploadImageToCloud(file.path);
...
exports.deleteActor = async (req, res) => {
  ...
  if (public_id) {
    ...
    if (result !== "ok") {
      sendError(res, "Could not remove image from cloud!");
    }
  }
...
exports.searchActor = async (req, res) => {
  ...
  if (!query.name) {
    return sendError(res, "Please provide the search Query!");
  }
...
exports.getActors = async (req, res) => {
  ...
  const profiles = result.map((actor) => formatActor(actor));
